...
#[hdk_extern]
pub fn genesis_self_checki(_data: GenesisSelfCheckData) -> ExternResult<ValidateCallbackResult> {
    Ok(ValidateCallbackResult::Valid)
}
    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        ...
        FlatOp::RegisterUpdate(update_entry) => match update_entry {
            OpUpdate::Entry { app_entry, action } => match app_entry {
                EntryTypes::Comment(comment) => validate_update_comment(action, comment),
                EntryTypes::Post(post) => validate_update_post(action, post),
            },
            _ => Ok(ValidateCallbackResult::Valid),
        },
        ...
        FlatOp::StoreRecord(store_record) => match store_record {
                ...
                match app_entry {
                    EntryTypes::Post(post) => {
                        ...
                        if let ValidateCallbackResult::Valid = result {
                            validate_update_post(action, post)
                        } else {
                            Ok(result)
                        }
                    }
                    EntryTypes::Comment(comment) => {
                        ...
                        if let ValidateCallbackResult::Valid = result {
                            validate_update_comment(action, comment)
                        } else {
                            Ok(result)
                        }
                    }
            ...
            OpRecord::DeleteEntry {
                ...
                match original_app_entry {
                    EntryTypes::Post(original_post) => validate_delete_post(action),
                    EntryTypes::Comment(original_comment) => validate_delete_comment(action),
                }
            }
            OpRecord::CreateLink {
                base_address,
